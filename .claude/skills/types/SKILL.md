...
2. **Read reference file**: Load `references/<lang>-types.md` for tool-specific instructions
...
7. **Commit incrementally**: Use message format `fix(types): resolve <error_code> in <file>`
...
- NEVER use `Any` (Python), `any` (TypeScript), or `object`/`dynamic` (.NET) without documented justification
